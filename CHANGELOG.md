...
- READMEを初心者向けに再構成
  - 「特徴」セクションを新設し、非公式API不使用・完全ローカル動作を明記
  - 「機能概要」を「ファイル構成」にリネームし、開発者向け参照情報として使い方の後ろへ移動
  - 使い方の先頭に「プロジェクトをダウンロードする」手順を追加
  - スクリーンショット取得方法にWin+PrtScn等のWindows標準手順を追記
  - `corrections.json`の編集手順に、メモ帳での編集方法・マージ時の具体例・構文エラー時の
    復旧方法を追加
  - 動作要件から実行後の注意事項を使い方の補足へ移動し、地の文の羅列を箇条書きに整理
